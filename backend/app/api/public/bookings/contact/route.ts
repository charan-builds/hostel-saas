...
export async function POST(request: NextRequest) {
  ...
  const log = createRequestLogger({
    event_type: "booking.contact.create",
  ...
  try {
    ...
    const rateLimitResponse = await enforceRequestRateLimit(request, {
      keyPrefix: "public:booking:contact",
      limit: 20,
    });
    ...
    const input = validateInput(createPublicBookingRequestSchema, {
      ...body,
    ...
    log.info(
      ...
      "Public booking contact created",
    );
    ...
    log.warn({ error }, "Public booking contact failed");
